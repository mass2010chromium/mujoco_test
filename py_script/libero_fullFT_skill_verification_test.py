"""
This script test the full finetuned model to operate with the help of our scene graph verifier. Namely it does the following
1) the model outputs a plan given the prompt at t = 0
2) scene graph verifier symbolically verifies the plan at t = 0
3) given the plan, output a skill to execute at t >= 0
4) if the skill differs from current skill, this is a skill transition, so verifier verifies the transition
5) if verification passes, move onto the next skill
6) the model outputs actions based on selected skill at t >= 0

NOTE: some slight modifications might be made but the above is the overall purpose
"""
import pathlib
import math
import os
...
if __name__ == "__main__":
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(TRIALS_PER_TASK)):
            ...
            scene_graph_hint = [f"Task Instruction: {task_description}"]
            scene_graph = TaskSceneGraph(pddl_domain_text, vlm_interface)
            scene_graph.read_image(cv2.cvtColor(cv2.imread(IMAGE_PATH), cv2.COLOR_BGR2RGB), hint=scene_graph_hint, ground=True)
            ...
            # scene_plan = '1. CLOSE(bottom drawer) 2. OPEN(top drawer) 3. PICKUP_FROM(black bowl, table) 4. PLACE_IN(black bowl, top drawer)'
            scene_plan = '1. CLOSE(bottom drawer) 2. OPEN(top drawer)'
                ...
                if trajectory_idx == (len(actions) // 2):
                    ...
                    if infer_count % think_freq == 0:
                        ...
                        if current_skill != skill:
                            ...
                            next_skill_idx = cur_skill_idx + 1
                            if next_skill_idx >= len(plan_steps):
                                next_skill_idx = len(plan_steps) - 1
                            skill = plan_steps[next_skill_idx]
