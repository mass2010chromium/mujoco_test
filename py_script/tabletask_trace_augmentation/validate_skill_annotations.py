#!/usr/bin/env python3
"""
Structural validator for tabletop manipulation skill-annotation JSON files.

This script checks:
- that each episode has a recoverable numbered plan
- that segment skill sequences agree with that plan
- that individual skill expressions are syntactically valid and restricted to
  the tabletop skill set {PICKUP_FROM, PLACE_ON, PLACE_IN}
...
- that the first planned skill is not one of the forbidden skill types
"""
...
PICKUP_SKILL_NAMES: frozenset[str] = frozenset({"PICKUP_FROM"})
PLACE_SKILL_NAMES: frozenset[str] = frozenset({"PLACE_ON", "PLACE_IN"})


def _skill_category(skill_expr: str) -> str:
    name = skill_expr.split("(", 1)[0]
    if name in PICKUP_SKILL_NAMES:
        return "pickup"
    if name in PLACE_SKILL_NAMES:
        return "place"
    return "other"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate skill-annotation JSON files by checking plan recoverability, segment-plan "
            "consistency, skill-expression validity, stray skill mentions, and the forbidden first "
            "skill rule."
        )
    )
    parser.add_argument(
...
def resolve_episode_plan(episode: dict[str, Any]) -> str:
    ...
    for idx, segment_plan in enumerate(segment_plans[1:], start=1):
        if parse_plan_skills(segment_plan) != parse_plan_skills(first_plan):
            raise ValueError(
                f"Segment plan strings disagree within the same trajectory at segment-plan index {idx}."
            )
    return first_plan
...
def validate_episode(episode: dict[str, Any], source: Path) -> list[ValidationFailure]:
    ...
    if not isinstance(segments, list) or not segments:
        return [
            ValidationFailure(
                ...
                "Missing or empty segments list.",
            )
        ]
    ...
    last_skill_name = plan_skills[-1].split("(", 1)[0]
    if last_skill_name not in PLACE_SKILL_NAMES:
        failures.append(
            ValidationFailure(
                source,
                int(episode_index) if isinstance(episode_index, int) else None,
                "The last planned skill must be a place skill "
                f"({sorted(PLACE_SKILL_NAMES)}); got {plan_skills[-1]!r}.",
            )
        )

    for prev_idx, (prev_skill, next_skill) in enumerate(zip(plan_skills, plan_skills[1:])):
        prev_category = _skill_category(prev_skill)
        next_category = _skill_category(next_skill)
        if prev_category == "other" or next_category == "other":
            continue
        if prev_category == next_category:
            expected = "place" if prev_category == "pickup" else "pickup"
            failures.append(
                ValidationFailure(
                    source,
                    int(episode_index) if isinstance(episode_index, int) else None,
                    f"Plan skills must alternate pickup and place. "
                    f"Plan item {prev_idx + 1} ({prev_skill!r}) is followed by "
                    f"item {prev_idx + 2} ({next_skill!r}); expected a {expected} skill.",
                )
            )
    ...
    # Validate segment contiguity / range against num_steps when present.
    ...
    for seg_idx, segment in enumerate(segments):
        ...
        if "start_step" in segment and "end_step" in segment:
            try:
                ...
                failures.append(
                    ValidationFailure(
                        source,
                        int(episode_index) if isinstance(episode_index, int) else None,
                        f"Segment {seg_idx} has non-integer start/end steps.",
                    )
                )
                start_step = end_step = None
            if start_step is not None:
                if seg_idx == 0 and start_step != 0:
                    failures.append(
                        ValidationFailure(
                            source,
                            int(episode_index) if isinstance(episode_index, int) else None,
                            f"First segment must start at step 0, got {start_step}.",
                        )
                    )
                if prev_end is not None and start_step != prev_end:
                    failures.append(
                        ValidationFailure(
                            source,
                            int(episode_index) if isinstance(episode_index, int) else None,
                            f"Segment {seg_idx} starts at {start_step}, expected contiguous {prev_end}.",
                        )
                    )
                    ...
                    if end_step <= start_step:
                        failures.append(
                            ValidationFailure(
                                source,
                                int(episode_index) if isinstance(episode_index, int) else None,
                                f"Segment {seg_idx} has non-positive length: [{start_step}, {end_step}).",
                            )
                        )
                    prev_end = end_step
    ...
    if isinstance(num_steps, int) and prev_end is not None and prev_end != num_steps:
        failures.append(
            ValidationFailure(
                source,
                int(episode_index) if isinstance(episode_index, int) else None,
                f"Last segment end_step {prev_end} does not equal num_steps {num_steps}.",
            )
        )
        ...
        for episode in episodes:
            ...
            if episode_failures:
                ...
                if args.stop_on_first_error:
                    print(
                        f"checked_files={total_files} checked_episodes={total_episodes} "
