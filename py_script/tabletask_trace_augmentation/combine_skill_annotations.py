...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Combine episode-wise tabletop annotation shard JSON files into a canonical skill annotation "
            "JSON and a training-oriented cot-style skill annotation JSON."
        )
    )
    parser.add_argument(
        "shard_dirs",
    ...
    parser.add_argument(
        ...
        help="Annotation playback FPS to record into the combined outputs.",
    )
    parser.add_argument(
        "--start-episode",
        type=int,
...
def main() -> int:
    ...
    if missing and not args.allow_missing:
        ...
        raise ValueError(
            f"Missing {len(missing)} episode shard(s) in the requested range "
            f"[{args.start_episode}, {end_episode}). Sample: {sample}"
        )
