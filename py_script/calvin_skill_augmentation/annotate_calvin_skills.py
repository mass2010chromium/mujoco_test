...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Annotate LeRobot-formatted CALVIN episodes with skill segments by querying "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable saving clean frames for each skill-boundary transition under "
...
def call_openrouter(
    *,
    ...
    headers = {
        ...
        "X-Title": "calvin-skill-augmentation",
    }
    request = Request(
        OPENROUTER_URL,
...
def annotate_episode(
    *,
    ...
    from common import normalize_model_steps

    prompt = build_multimodal_prompt(record)
...
def main() -> int:
    ...
    for episode_index in range(start_episode, end_episode):
            ...
            if episode_transition_dir is not None and args.skip_existing and not args.overwrite_existing:
                try:
                    ...
                    if not all(path.exists() for path in expected_paths):
                        episode_rows = load_episode_rows(dataset_root, episode_index, dataset_info=dataset_info)
                        save_transition_scene_images(
                            episode_rows,
                            record=record,
                            segments=existing_episode["segments"],
        ...
        video_path = (
            ...
            if args.keep_videos
            else output_dir / f".episode_{episode_index:06d}.mp4"
        )
        ...
        print(
            ...
            f"episode={episode_index} task_index={record.task_index} task_name={record.task_name} length={record.length}",
            flush=True,
        )
