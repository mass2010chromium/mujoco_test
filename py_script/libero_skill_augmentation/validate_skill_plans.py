...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate skill annotations with the symbolic scene-graph verifier. Supports "
    ...
    parser.add_argument(
        "--repo-id",
        default=None,
        help="Optional repo id override. Defaults to the annotation metadata, then to the standard LIBERO repo id.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pddl-path",
        type=Path,
        default=PY_SCRIPT_DIR / "pddl" / "libero_domain.pddl",
        help="PDDL domain used by the symbolic LIBERO verifier.",
    )
    parser.add_argument(
        "--backend",
...
def load_rgb_image(path: Path) -> Any:
    ...
    print("load path", path, flush=True)
...
def main() -> int:
    ...
    for path, episodes, file_metadata, transition_scene_roots in loaded_inputs:
        ...
        for episode in episodes:
            ...
            print(
                f"[{total_episodes}/{total_expected_episodes}] validating {path} "
            ...
            print(
                f"[FAIL] {path} episode={failure['episode_index']} stage={failure['failure_stage']}: "
                f"{failure.get('error') or failure.get('plan_validation', {}).get('failure_reason') or failure.get('transition_validation', {}).get('failure_reason')}",
                flush=True,
            )
            if args.stop_on_first_error:
                summary = {
                    "created_at": utc_now(),
                    "inputs": [str(input_path.expanduser().resolve()) for input_path in args.inputs],
                    "pddl_path": str(pddl_path),
                    "backend": args.backend,
                    "plan_only": args.plan_only,
                    "start": args.start,
                    "end": args.end,
                    "transition_scene_roots": [str(path) for path in explicit_transition_scene_roots],
                    "checked_files": total_files,
                    "checked_episodes": total_episodes,
                    "failed_episodes": len(failures),
                    "successful_episodes": total_episodes - len(failures),
                    "failures": failures,
                }
                save_json_atomic(output_path, summary)
                print(
