...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Visualize saved LIBERO skill-transition frames one at a time. The script writes "
            f"{OUTPUT_IMAGE_NAME} in the current working directory and overwrites it on each iteration."
        )
    )
    parser.add_argument(
        "run_dir",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Traverse transitions sequentially from the beginning. When omitted, each iteration samples "
...
) -> tuple[list[TransitionExample], dict[int, list[TransitionExample]]]:
    ordered: list[TransitionExample] = []
    by_episode: dict[int, list[TransitionExample]] = {}

    for episode_index in sorted(episodes):
        episode = episodes[episode_index]
        canonical_segments = canonical_segments_from_episode(episode)
        if len(canonical_segments) < 2:
            continue
        ...
        for transition_index in range(1, len(canonical_segments)):
            ...
            example = TransitionExample(
                ...
                previous_segment_index=transition_index - 1,
                next_segment_index=transition_index,
            ...
            by_episode.setdefault(episode_index, []).append(example)

    if not ordered:
        raise ValueError("No skill transitions were found in the provided run directory.")

    return ordered, by_episode
...
def render_overlay(example: TransitionExample, output_path: Path) -> None:
...
def print_example(example: TransitionExample, output_path: Path) -> None:
    print(f"episode_index: {example.episode_index}", flush=True)
    print(f"segment_index: {example.next_segment_index}", flush=True)
...
def run_sequential_mode(examples: list[TransitionExample], output_path: Path) -> int:
    cursor = 0
    ...
    print("Reached the end of the sequential transition list.", flush=True)
    return 0


def run_random_mode(examples_by_episode: dict[int, list[TransitionExample]], output_path: Path) -> int:
    rng = random.Random()
    episode_indices = sorted(examples_by_episode)

    while True:
        episode_index = rng.choice(episode_indices)
        example = rng.choice(examples_by_episode[episode_index])
        render_overlay(example, output_path)
        print_example(example, output_path)
        if prompt_for_next() == "quit":
            return 0
...
def main() -> int:
    ...
    ordered_examples, examples_by_episode = build_transition_examples(
        episodes,
    ...
    print(f"episodes_with_transitions: {len(examples_by_episode)}", flush=True)
    print(f"total_transitions: {len(ordered_examples)}", flush=True)
    print(f"mode: {'sequential' if args.random_order else 'random'}", flush=True)
    print(flush=True)

    if args.random_order:
        return run_random_mode(examples_by_episode, output_path)
    return run_sequential_mode(ordered_examples, output_path)
