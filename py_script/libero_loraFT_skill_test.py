...
def prompt_from_obs(obs, task, scene_plan='', mode='thinking'):
    ...
    if mode == 'thinking' or scene_plan == '':
        ...
        skill = scene_plan.split("Now I need to do:", 1)[1].strip()
        print(f"Skill: {skill}")
        return {
            'observation/image': obs['agentview_image'][::-1, ::-1, :],
            'observation/wrist_image': obs['robot0_eye_in_hand_image'][::-1, ::-1, :],
...
if __name__ == "__main__":
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        if task_id != 8:       # test task 10 --> drawer closed initially
            continue
        ...
        # task_description = "open the drawer and pick up the black bowl and put it inside the drawer"
        ...
        for episode_idx in tqdm.tqdm(range(TRIALS_PER_TASK)):
            ...
            prompt = prompt_from_obs(obs, task_description, scene_plan, mode='acting')
            result = policy.infer(prompt)
            infer_count += 1
            ...
            for i in range(EPISODE_LENGTH):
                ...
                if trajectory_idx == (len(actions) // 2):
                    ...
                    if infer_count % think_freq == 0:
                        ...
                        if subtask is not None:
                            scene_plan = subtask
                        print(f"[Step {i}] Reasoning (thinking): {subtask}")
